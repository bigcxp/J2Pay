...
// 提领订单列表
func PickList(fromDate string, toDate string, status int, name string, code string, page, pageSize int) (res model.PickUpPage, err error) {
	pick := model.Pick{}
	if name == "" && status == 0 && fromDate == "" && toDate == "" && code == "" {
		res, err = pick.GetAll(page, pageSize)
	} else {
		//将时间进行转换
		res, err = pick.GetAll(page, pageSize, "status = ? or real_name like ? or is_code like ? or UNIX_TIMESTAMP(created_at)>=? or  UNIX_TIMESTAMP(created_at) <=?", status, name, code, fromDate, toDate)
	}
	return
}
