...
const (
	...
	DBColTAddressKeyUseTag  = "t_address_key.use_tag" // 占用标志 -1 作为热钱包占用-0 未占用->0 作为用户充币地址占用
)

// TAddressKey t_address_key 数据表
/*
   id,
   symbol,
   address,
...
// TAppConfigInt t_app_config_int 数据表
/*
   id,
   k,
   v
...
// TAppConfigToken t_app_config_token 数据表
/*
   id,
   token_address,
   token_decimals,
...
*/
//比特币 暂时不用
type TAppConfigTokenBtc struct {
	ID              int64  `gorm"id" json:"id"`
	TokenIndex      int64  `gorm"token_index" json:"token_index"`
	TokenSymbol     string `gorm"token_symbol" json:"token_symbol"`
	ColdAddress     string `gorm"cold_address" json:"cold_address"`
	HotAddress      string `gorm"hot_address" json:"hot_address"`
	FeeAddress      string `gorm"fee_address" json:"fee_address"`
	TxOrgMinBalance string `gorm"tx_org_min_balance" json:"tx_org_min_balance"`
	CreateAt        int64  `gorm"create_at" json:"create_at"`
}
...
type DBTAppStatusInt struct {
	ID int64  `gorm"id" json:"id"`
	K  string `gorm"k" json:"k"` // 配置键名
	V  int64  `gorm"v" json:"v"` // 配置键值
}
...
type DBTProduct struct {
	ID          int64  `gorm"id" json:"id"`
	AppName     string `gorm"app_name" json:"app_name"`         // 应用名
	AppSk       string `gorm"app_sk" json:"app_sk"`             // 应用私钥
	CbURL       string `gorm"cb_url" json:"cb_url"`             // 回调地址
	WhitelistIP string `gorm"whitelist_ip" json:"whitelist_ip"` // ip白名单
}
...
type DBTProductNotify struct {
	ID           int64  `gorm"id" json:"id"`
	Nonce        string `gorm"nonce" json:"nonce"`
	ProductID    int64  `gorm"product_id" json:"product_id"`
	ItemType     int64  `gorm"item_type" json:"item_type"`
	ItemID       int64  `gorm"item_id" json:"item_id"`
	NotifyType   int64  `gorm"notify_type" json:"notify_type"`
	TokenSymbol  string `gorm"token_symbol" json:"token_symbol"`
	URL          string `gorm"url" json:"url"`
	Msg          string `gorm"msg" json:"msg"`
	HandleStatus int64  `gorm"handle_status" json:"handle_status"`
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`
	CreateTime   int64  `gorm"create_time" json:"create_time"`
...
type DBTSend struct {
	ID           int64  `gorm"id" json:"id"`
	RelatedType  int64  `gorm"related_type" json:"related_type"` // 关联类型 1 零钱整理 2 提币
	RelatedID    int64  `gorm"related_id" json:"related_id"`     // 关联id
	TokenID      int64  `gorm"token_id" json:"token_id"`
	TxID         string `gorm"tx_id" json:"tx_id"`                 // tx hash
	FromAddress  string `gorm"from_address" json:"from_address"`   // 打币地址
	ToAddress    string `gorm"to_address" json:"to_address"`       // 收币地址
	BalanceReal  string `gorm"balance_real" json:"balance_real"`   // 打币金额 Ether
	Gas          int64  `gorm"gas" json:"gas"`                     // gas消耗
	GasPrice     int64  `gorm"gas_price" json:"gas_price"`         // gasPrice
	Nonce        int64  `gorm"nonce" json:"nonce"`                 // nonce
	Hex          string `gorm"hex" json:"hex"`                     // tx raw hex
	...
	HandleTime   int64  `gorm"handle_time" json:"handle_time"`     // 处理时间
}
...
type DBTSendEos struct {
	ID           int64  `gorm"id" json:"id"`
	WithdrawID   int64  `gorm"withdraw_id" json:"withdraw_id"`     // 关联id
	TxHash       string `gorm"tx_hash" json:"tx_hash"`             // tx hash
	LogIndex     int64  `gorm"log_index" json:"log_index"`         // log_index
	FromAddress  string `gorm"from_address" json:"from_address"`   // 打币地址
	ToAddress    string `gorm"to_address" json:"to_address"`       // 收币地址
	Memo         string `gorm"memo" json:"memo"`                   // 收币地址
	BalanceReal  string `gorm"balance_real" json:"balance_real"`   // 打币金额 Ether
	Hex          string `gorm"hex" json:"hex"`                     // tx raw hex
	CreateTime   int64  `gorm"create_time" json:"create_time"`     // 创建时间
	HandleStatus int64  `gorm"handle_status" json:"handle_status"` // 处理状态
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`       // 处理消息
	HandleAt     int64  `gorm"handle_at" json:"handle_at"`         // 处理时间
}
...
type DBTTx struct {
	ID           int64  `gorm"id" json:"id"`
	ProductID    int64  `gorm"product_id" json:"product_id"`
	TxID         string `gorm"tx_id" json:"tx_id"`                 // 交易id
	FromAddress  string `gorm"from_address" json:"from_address"`   // 来源地址
	ToAddress    string `gorm"to_address" json:"to_address"`       // 目标地址
	BalanceReal  string `gorm"balance_real" json:"balance_real"`   // 到账金额Ether
	CreateTime   int64  `gorm"create_time" json:"create_time"`     // 创建时间戳
	HandleStatus int64  `gorm"handle_status" json:"handle_status"` // 处理状态
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`       // 处理消息
...
type DBTTxBtc struct {
	...
	VoutN        int64  `gorm"vout_n" json:"vout_n"`
	VoutAddress  string `gorm"vout_address" json:"vout_address"`
	VoutValue    string `gorm"vout_value" json:"vout_value"`
	CreateTime   int64  `gorm"create_time" json:"create_time"`
	HandleStatus int64  `gorm"handle_status" json:"handle_status"`
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`
	HandleTime   int64  `gorm"handle_time" json:"handle_time"`
}
...
type DBTTxBtcToken struct {
	...
	ToAddress    string `gorm"to_address" json:"to_address"`
	Value        string `gorm"value" json:"value"`
	Blocktime    int64  `gorm"blocktime" json:"blocktime"`
	CreateAt     int64  `gorm"create_at" json:"create_at"`
	HandleStatus int64  `gorm"handle_status" json:"handle_status"`
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`
	HandleAt     int64  `gorm"handle_at" json:"handle_at"`
	OrgStatus    int64  `gorm"org_status" json:"org_status"`
	OrgMsg       string `gorm"org_msg" json:"org_msg"`
	OrgAt        int64  `gorm"org_at" json:"org_at"`
}
...
type DBTTxBtcUxto struct {
	ID           int64  `gorm"id" json:"id"`
	UxtoType     int64  `gorm"uxto_type" json:"uxto_type"`
	BlockHash    string `gorm"block_hash" json:"block_hash"`
	TxID         string `gorm"tx_id" json:"tx_id"`
	VoutN        int64  `gorm"vout_n" json:"vout_n"`
	VoutAddress  string `gorm"vout_address" json:"vout_address"`
	VoutValue    string `gorm"vout_value" json:"vout_value"`
	VoutScript   string `gorm"vout_script" json:"vout_script"`
	CreateTime   int64  `gorm"create_time" json:"create_time"`
	SpendTxID    string `gorm"spend_tx_id" json:"spend_tx_id"`
	SpendN       int64  `gorm"spend_n" json:"spend_n"`
	HandleStatus int64  `gorm"handle_status" json:"handle_status"`
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`
	HandleTime   int64  `gorm"handle_time" json:"handle_time"`
}
...
type DBTTxEos struct {
	...
	LogIndex     int64  `gorm"log_index" json:"log_index"`
	FromAddress  string `gorm"from_address" json:"from_address"`
	ToAddress    string `gorm"to_address" json:"to_address"`
	Memo         string `gorm"memo" json:"memo"`
	BalanceReal  string `gorm"balance_real" json:"balance_real"`
	CreateAt     int64  `gorm"create_at" json:"create_at"`
	HandleStatus int64  `gorm"handle_status" json:"handle_status"`
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`
	HandleAt     int64  `gorm"handle_at" json:"handle_at"`
...
type DBTWithdraw struct {
	ID           int64  `gorm"id" json:"id"`
	ProductID    int64  `gorm"product_id" json:"product_id"` // 产品id
	OutSerial    string `gorm"out_serial" json:"out_serial"` // 提币唯一标示
	ToAddress    string `gorm"to_address" json:"to_address"` // 提币地址
	Memo         string `gorm"memo" json:"memo"`
	Symbol       string `gorm"symbol" json:"symbol"`
	BalanceReal  string `gorm"balance_real" json:"balance_real"`   // 提币金额
	TxHash       string `gorm"tx_hash" json:"tx_hash"`             // 提币tx hash
	CreateTime   int64  `gorm"create_time" json:"create_time"`     // 创建时间
	HandleStatus int64  `gorm"handle_status" json:"handle_status"` // 处理状态
	HandleMsg    string `gorm"handle_msg" json:"handle_msg"`       // 处理消息
	HandleTime   int64  `gorm"handle_time" json:"handle_time"`     // 处理时间
