...
func Cron() {
	go func() {
		...
		// 检测 eth 冲币
		_, err = c.AddFunc("@every 2m", heth.CheckBlockSeek)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		...
		// 检测 eth 交易上链=》是否完成打包
		_, err = c.AddFunc("@every 30s", heth.CheckRawTxConfirm)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		// 检测 eth 通知到账
		_, err = c.AddFunc("@every 40s", heth.CheckTxNotify)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		// 检测 eth gas price
		...
		// --- erc20 ---
		// 检测 erc20 冲币
		_, err = c.AddFunc("@every 2m", heth.CheckErc20BlockSeek)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		// 检测 erc20 通知到账
		_, err = c.AddFunc("@every 40s", heth.CheckErc20TxNotify)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		// 检测 erc20 零钱整理
		...
		// 检测 erc20 提币
		_, err = c.AddFunc("@every 5s", heth.CheckErc20Withdraw)
		if err != nil {
			log.Panicf("cron add func error: %#v", err)
		}
		c.Start()
